...
public class AccountMenu {
	...
	public AccountMenu(Player player, BattlePlayer opener, BattlePlayer battlePlayer) {
		...
		loreBuilder = new StringBuilder();
		loreBuilder.append("§%actual-group%§: "
				+ Tag.valueOf(battlePlayer.getServerGroup().toString()).getPrefix(opener.getLanguage()));
		if (battlePlayer.getRanks().size() > 0) {
			loreBuilder.append("\n");
			for (Entry<RankType, Long> entry : battlePlayer.getRanks().entrySet()) {
				loreBuilder.append("\n" + Tag.valueOf(entry.getKey().toString()).getPrefix(opener.getLanguage())
						+ ChatColor.RESET + " - " + DateUtils.formatDifference(opener.getLanguage(),
		...
		if (battlePlayer.getGroups().size() > 0) {
			...
			for (Entry<ServerStaff, Group> staff : battlePlayer.getGroups().entrySet()) {
				loreBuilder.append("\n" + ChatColor.WHITE + ChatColor.BOLD + staff.getKey().toString() + " - "
						+ Tag.valueOf(staff.getValue().toString()).getPrefix(opener.getLanguage()));
			}
		}
		...
		menu.setItem(51,
				new MenuItem(new ItemBuilder().type(Material.BARRIER).name("§%bans%§").lore("§%bans-lore%§").build(),
		...
		menu.setItem(52, new MenuItem(
				new ItemBuilder().type(Material.BOOK_AND_QUILL).name("§%mutes%§").lore("§%mutes-lore%§").build(),
				new MenuClickHandler() {
