...
public class GroupCommand implements CommandClass {
	...
	public void groupset(BungeeCommandArgs cmdArgs) {
		...
		final String groupSetPrefix = T.t(BungeeMain.getPlugin(),lang, "command-groupset-prefix") + " ";
		if (args.length != 2) {
			sender.sendMessage(TextComponent.fromLegacyText(groupSetPrefix + T.t(BungeeMain.getPlugin(),lang, "command-groupset-usage")));
			return;
		}
		Group grupo = null;
		try {
			...
			sender.sendMessage(TextComponent.fromLegacyText(groupSetPrefix + T.t(BungeeMain.getPlugin(),lang, "command-groupset-group-not-exist")));
			return;
		}
		...
		if (group.ordinal() <= Group.YOUTUBER.ordinal() && group != Group.NORMAL) {
			sender.sendMessage(TextComponent.fromLegacyText(groupSetPrefix + T.t(BungeeMain.getPlugin(),lang, "command-groupset-group-temporary")));
			return;
		}

		if (!owner)
			if (group.ordinal() > Group.MODPLUS.ordinal()) {
				sender.sendMessage(TextComponent.fromLegacyText(groupSetPrefix + T.t(BungeeMain.getPlugin(),lang, "command-groupset-group-not-owner")));
				return;
			}
		...
		if (actualGroup == group) {
			sender.sendMessage(TextComponent.fromLegacyText(groupSetPrefix + T.t(BungeeMain.getPlugin(),language, "command-groupset-player-already-group")));
			return;
		}
		...
		String message = groupSetPrefix + T.t(BungeeMain.getPlugin(),language, "command-groupset-change-group");
